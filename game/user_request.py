...
class TowerDeveloper:
    ...
    def update(self, user_request: str, model):
        ...
        if user_request == "upgrade" and model.selected_tower.level < 5: # if click upgrade and the tower level isn't exceeded
            ...
            if model.money >= model.selected_tower.get_upgrade_cost():  # if money is enough to upgrade 
                x, y = model.selected_tower.rect.center                 # get the position to know which tower we are choosing
                model.money -= model.selected_tower.get_upgrade_cost()  # spend the money to upgrade 
                model.selected_tower.level += 1                         # tower level +1

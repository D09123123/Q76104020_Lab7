...
class EnemyGroup:
    ...
    def advance(self, model):
        ...
        self.campaign()                                # run the campaign function
        for en in self.__expedition:                   # for the enemies on board 
            en.move()
            if en.health <= 0:                         # if the enemy's hp <= 0, which is executed by tower
                self.retreat(en)                       # retreat the enemy, enemy vanish
                model.money += 15                      # get the reward by killing the enemy, $15 for each enemy
            # delete the object when it reach the base
            if BASE.collidepoint(en.rect.centerx, en.rect.centery): # if the enemy reaches the ES base         
                self.retreat(en)                                    # retreat the enemy, enemy vanish
                model.hp -= 1                                       # the base's hp will -1 (attacked by the enemy)
